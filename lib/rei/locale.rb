#
#   rei/locale.rb - internationalization module
#		    Original from irb.
...
module REI
  class Locale
    ...
    def initialize(locale = nil)
      ...
      @locale = locale || ENV["REISH_LANG"] || ENV["LC_MESSAGES"] || ENV["LC_ALL"] || ENV["LANG"] || "C"
      if m = LOCALE_NAME_RE.match(@locale)
        @lang, @territory, @encoding_name, @modifier = m[:language], m[:territory], m[:codeset], m[:modifier]

        if @encoding_name
          begin load 'reish/encoding_aliases.rb'; rescue LoadError; end
    ...
    # @param paths load paths in which Reish find a localized file.
    # @param dir directory
